...
export default {
    ...
    ADD_COMPLAINT: async (req, res) => {
        try {
            const { complaint, description, urgency,user_id,ID,first_work,devices,platforms,last_day,return_device,fullname } = req.body;
            const { user} = req.query
            await addComplaint(complaint, description, urgency,user,ID,first_work,devices,platforms,last_day,return_device,fullname);
            res.send(await getComplaints());
        } catch (error) {
            res.send({ error: error.message, serverRes: 500 });
        }
    ...
    UPDATE_COMPLAINT: async (req, res) => {
        try {
            ...
            const { complaint, description, urgency,ID, first_work, devices, platforms, last_day, return_device, fullname } = req.body;
            ...
            await updateComplaint(complaint, description, urgency, user_id, ID, first_work, devices, platforms, last_day, return_device, fullname, +req.params.id);
            res.send(await getComplaints());
        } catch (error) {
            res.send({ error: error.message, serverRes: 500 });
        }
