...
export default createStore({
  ...
  actions: {
    ...
    async getTickets({ commit }) {
      try {
        let { data } = await axios.get(BASE_URL + "/tickets");
        console.log(data);
    ...
    async updateTicket({ commit }, update) {
      try {
        await axios.patch(BASE_URL + "/tickets/" + update.id, update);
        Swal.fire({
          title: "Update Successful",
          text: "Ticket has been update successfully!",
          icon: "success",
          timer: 3000,
          showConfirmButton: false,
        });
    ...
    //===should be a time limit window on when they create a ticket so they can delete the ticket in that time frame incase it was an accident====//

    async delTicket({ commit }, id) {
      try {
